...
use codec::{Block, Log, TransactionTrace, TransactionReceipt};
use prost::Message;
use archive::{Archive, BatchRequest};
use std::sync::Arc;
...
impl Stream for ArchiveStream {
    ...
    async fn blocks(&self, request: Request<firehose::Request>) -> Result<Response<Self::BlocksStream>, Status> {
        let mut fields = archive::FieldSelection {
            block: Some(archive::BlockFieldSelection {
        ...
        let mut logs: Vec<archive::LogRequest> = vec![];
        ...
        for transform in &request.get_ref().transforms {
            ...
            for log_filter in filter.log_filters {
                let log_request = archive::LogRequest {
                ...
                fields.log = Some(archive::LogFieldSelection {
                    address: true,
                    data: true,
                    log_index: true,
                    topics: true,
                ...
                fields.transaction = Some(archive::TxFieldSelection {
        ...
        tokio::spawn(async move {
            'outer: loop {
                ...
                for block in blocks {
                    ...
                    let mut logs_by_tx: std::collections::HashMap<u32, Vec<archive::Log>> = std::collections::HashMap::new();
                    ...
                    if let Some(transactions) = block.transactions {
                        graph_block.transaction_traces = transactions.into_iter().map(|tx| {
                            let logs = logs_by_tx.remove(&tx.transaction_index).unwrap_or_default().into_iter().map(|log| Log {
                    ...
                    let result = tx.send(Ok(firehose::Response {
